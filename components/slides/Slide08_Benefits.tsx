...
export function Slide08_Benefits() {
  return (
    <SlideFrame>
      ...
      <div style={{ position: "absolute", inset: 0, background: "radial-gradient(62% 58% at 50% 64%, rgba(5,13,28,0.88) 0%, rgba(5,13,28,0.42) 55%, rgba(5,13,28,0) 82%)" }} />
      {/* Eyebrow */}
      <div style={{ position: "absolute", left: 120, top: 38, width: 1040, height: 48, backgroundColor: "rgba(5,13,28,0.94)", border: "1px solid rgba(52,233,226,0.5)", boxShadow: "0 0 36.0px rgba(52,233,226,0.40)", display: "flex", alignItems: "center", padding: "0 20px" }}>
      ...
      {/* Main narrative card — centred, content-sized (no fixed height), and
          shifted lower so more of the background video reads through above it. */}
      <div style={{ position: "absolute", left: "50%", top: 300, width: 940, transform: "translateX(-50%)", backgroundColor: "rgba(5,13,28,0.93)", border: "1px solid rgba(52,233,226,0.52)", boxShadow: "0 0 36.0px rgba(52,233,226,0.34)", padding: "36px 46px 40px" }}>
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25, duration: 0.5 }}>
          <p style={{ textAlign: "center", margin: 0, padding: 0, lineHeight: 1.1, whiteSpace: "normal" }}><span style={{ fontSize: "50.0px", letterSpacing: "-0.6px", color: "#34E9E2" }}>Why</span><span style={{ fontSize: "50.0px", letterSpacing: "-0.6px", color: "#FFFFFF" }}> not charge?</span><br /><span style={{ fontSize: "50.0px", letterSpacing: "-0.6px", color: "#FFFFFF" }}>Because the </span><span style={{ fontSize: "50.0px", letterSpacing: "-0.6px", color: "#34E9E2" }}>network</span><span style={{ fontSize: "50.0px", letterSpacing: "-0.6px", color: "#FFFFFF" }}> is the value.</span></p>
        </motion.div>
        <div style={{ display: "grid", gap: 16, marginTop: 24 }}>
          {BENEFITS.map(({ title, body, accent }, index) => (
            <motion.div
              ...
              style={{ display: "grid", gridTemplateColumns: "20px 1fr", gap: 16, alignItems: "start" }}
            >
              <span style={{ width: 8, height: 8, marginTop: 10, borderRadius: 999, background: accent ?? "#34E9E2", boxShadow: `0 0 18px ${accent ?? "#34E9E2"}` }} />
              <p style={{ textAlign: "left", margin: 0, padding: 0, lineHeight: 1.16, whiteSpace: "normal" }}>
                <span style={{ fontSize: "22.0px", fontWeight: 700, color: accent ?? "#34E9E2" }}>{title}</span>
                <span style={{ fontSize: "22.0px", fontWeight: 700, color: "#FFFFFF" }}> - {body}</span>
              </p>
            </motion.div>
          ))}
        </div>
